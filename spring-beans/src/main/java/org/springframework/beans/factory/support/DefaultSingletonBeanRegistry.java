...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
    ...
    protected Object getSingleton(String beanName, boolean allowEarlyReference) {
        // 这里由三级缓存的概念
        /**
         * 第一级缓存singletonObjects，里面存放最终的bean
         * 第二级缓存earlySingletonObjects ，里面往往存放着bean的半成品（未完成属性注入的），或者说是对象
         * 第三级缓存singletonFactories, 里面存放用于生成bean的半成品的工厂
         *
         * spring中实现循环引用的一个很重要的思想就是通过提前暴露bean的半成品，但是为什么要有第三级的缓存呢？
         * 而不是直接生成半成品放入到earlySingletonObjects中呢？原因是为了实现被循环引用的对象的AOP增强
         */
     ...
     * @since 4.0
     */
    //dependentBeanName 是否依赖于beanName
